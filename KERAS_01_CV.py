...
for d in range(39):
    ...
    if not os.path.isfile('predictions/train/' + file_name + '.npy'):
        ...
        yte = np.log(ytrain[inTe] + 1)

        tr = []
        te = []
        for f in feat:
            x = np.load('features/train/' + f + '.npy')
            x[np.isnan(x)] = 0
        ...
        unique_id = np.load('features/train/unique_id.npy')
        month = np.load('features/train/month.npy')
        week = np.load('features/train/week.npy')
        day = np.load('features/train/day.npy')
        weekday = np.load('features/train/weekday.npy')
        air_genre_name = np.load('features/train/air_genre_name.npy')
        air_area_name = np.load('features/train/air_area_name.npy')
        hpg_genre_name = np.load('features/train/hpg_genre_name.npy')
        hpg_area_name = np.load('features/train/hpg_genre_name.npy')
        ...
        n_hpg_genre_name = len(np.unique(hpg_genre_name)) + 1
        n_hpg_area_name = len(np.unique(hpg_area_name)) + 1
        ...
        for i in range(nbag):
            ...
            model.fit([unique_id[inTr], month[inTr], week[inTr], day[inTr], weekday[inTr], air_genre_name[inTr], air_area_name[inTr], tr], ytr, 
                      ...
                      epochs = 25,
                      validation_data = ([unique_id[inTe], month[inTe], week[inTe], day[inTe], weekday[inTe], air_genre_name[inTe], air_area_name[inTe], te], yte))
            pred += model.predict([unique_id[inTe], month[inTe], week[inTe], day[inTe], weekday[inTe], air_genre_name[inTe], air_area_name[inTe], te])[:,0]
            error = np.sqrt(mean_squared_error(yte, pred/(i+1)))
            print('RMSE: {0}' .format(np.round(error, 6)))
